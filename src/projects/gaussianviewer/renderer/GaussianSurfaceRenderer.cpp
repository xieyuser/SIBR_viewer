...
namespace sibr
{
		...
		float *seg_data)
	{
		...
		glCreateBuffers(1, &contriBuffer);
		...
		glNamedBufferStorage(contriBuffer, num_gaussians * sizeof(float), contri_data, 0);
		glNamedBufferStorage(colorBuffer, num_gaussians * 3 * sizeof(float), color_data, 0);
		glNamedBufferStorage(segBuffer, num_gaussians * 15 * sizeof(float), seg_data, 0);
	}
	...
	void GaussianData::render(int G) const
	{
		...
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, contriBuffer);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 5, colorBuffer);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 6, segBuffer);
